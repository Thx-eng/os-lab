               ...
               bool share)
{
    ...
    do
    {
        ...
        //      uint32_t perm = (*ptep) & (PTE_USER | PTE_R | PTE_W | PTE_X);
        //     // get page from ptep
        //     struct Page *page = pte2page(*ptep);
        ...
        //     /* replicate page contents */
        //     memcpy(page2kva(npage), page2kva(page), PGSIZE);

        //     /* install mapping into destination pgdir */
        ...
        if (*ptep & PTE_V)
        {
            ...
            // uint32_t perm = (*ptep) & (PTE_USER | PTE_R | PTE_W | PTE_X);
            // struct Page *src = pte2page(*ptep);
            // assert(src != NULL);

            // struct Page *dst = alloc_page();
            // if (dst == NULL)
            // {
            //     return -E_NO_MEM;
            // }
            ...
            // /* 复制页面内容 */
            // memcpy(page2kva(dst), page2kva(src), PGSIZE);

            // /* 在目标页表中安装映射，出现错误则释放新页并返回 */
            // if (page_insert(to, dst, start, perm) != 0)
            // {
            //     free_page(dst);
            //     return -E_NO_MEM;
            // }
            ...
            if ((perm & PTE_W) || (perm & PTE_COW)) {
                ...
                // 修改父进程的页表项：设为只读，标记 COW
                *ptep = pte_create(page2ppn(page), perm | PTE_V);
                // 此时父进程的 TLB 可能已过时，需要刷新 (fork 期间父进程在运行)
                tlb_invalidate(from, start);
            }
            
            // 将相同的物理页映射到子进程，权限相同（只读+COW）
            // page_insert 会自动增加物理页的引用计数 (page->ref)
