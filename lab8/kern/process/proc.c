...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (i = 0; i < elf.e_phnum; i++)
    {
        ...
        if (!USER_ACCESS(ph.p_va, ph.p_va + ph.p_memsz))
        {
            ret = -E_INVAL_ELF;
            goto bad_cleanup_mmap;
        }
        ...
        uintptr_t la = ROUNDDOWN(ph.p_va, PGSIZE);
        uintptr_t la_end = ROUNDUP(ph.p_va + ph.p_memsz, PGSIZE);
        if ((ret = mm_map(mm, la, la_end - la, vm_flags, NULL)) != 0)
        {
            goto bad_cleanup_mmap;
        }
        ...
        uintptr_t mem_end = ph.p_va + ph.p_memsz;
        size_t pageoff = start - la;
        ...
        while (start < mem_end)
        {
            ...
            void *kva = page2kva(page) + pageoff;
            size_t page_left = PGSIZE - pageoff;

            size_t to_copy = 0;
            if (start < file_end)
            {
                to_copy = file_end - start;
                if (to_copy > page_left)
                {
                    to_copy = page_left;
                }
                if ((ret = load_icode_read(fd, kva, to_copy, offset)) != 0)
                {
                    goto bad_cleanup_mmap;
                }
                offset += to_copy;
            }
            if (page_left > to_copy)
            {
                memset((char *)kva + to_copy, 0, page_left - to_copy);
            }
    ...
    for (uintptr_t va = USTACKTOP - USTACKSIZE; va < USTACKTOP; va += PGSIZE)
    {
        if ((page = pgdir_alloc_page(mm->pgdir, va, PTE_U | PTE_R | PTE_W)) == NULL)
        {
            ret = -E_NO_MEM;
            goto bad_cleanup_mmap;
        }
    ...
    int i_arg;
    for (i_arg = argc - 1; i_arg >= 0; i_arg--)
    {
        size_t len = strlen(kargv[i_arg]) + 1;
        sp -= len;
        memcpy((void *)sp, kargv[i_arg], len);
        uargv[i_arg] = sp;
    }
    uargv[argc] = 0;
    sp = ROUNDDOWN(sp, 16);
    sp -= (argc + 1) * sizeof(uintptr_t);
    memcpy((void *)sp, uargv, (argc + 1) * sizeof(uintptr_t));
