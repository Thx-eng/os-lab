  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	...
	*/
    ...
    while (nblks != 0) {
        ...
        while (nstep < nblks) {
            if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno + nstep, &ino_next)) != 0) {
                goto out;
            }
            if (ino_next != ino + nstep) {
                break;
            }
            nstep ++;
        }
        if ((ret = sfs_block_op(sfs, buf, ino, nstep)) != 0) {
            goto out;
        }
...
out:
    *alenp = alen;
    if (start + alen > sin->din->size) {
        sin->din->size = start + alen;
        sin->dirty = 1;
    }
    return ret;
}
